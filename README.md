...
tifa 工具贯彻的是，看到什么写什么，一切依据视觉逻辑。由于元素定位依赖的并不是 xpath、id 等实际元素标识，因而解决了传统 UI 测试的以下痛点：

- 理想情况下，只需对着 UI 图甚至仅凭想象写出测试用例。
...
描述一个元素的基本语法是：`[Type "label" { condition }] = Nick`
其中 Type 是元素的类型；label 是元素的值，如果为空，可以不写；Nick 是给这个元素起的别名，方便在后续引用。
...
不用的元素类型会对应不同的动作，它基本符合人类的交互习惯，例如我们不会在 button 类型的元素上进行输入的操作，而应该是 click、hover 这样的操作。
